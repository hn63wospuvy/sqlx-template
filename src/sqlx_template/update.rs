        ...
        })) = attr.parse_meta()
        {
            if path.is_ident("tp_update") {
                        ...
                        NestedMeta::Meta(Meta::NameValue(nv)) => {
                            if nv.path.is_ident("by") {
                                ...
                                if let Lit::Str(lit) = &nv.lit {
                                    ...
                                    let fields_str =
                                        lit.split(',').map(|x| x.trim()).collect::<Vec<_>>();
                                    on_fields = super::check_fields(&fields_str, all_fields.clone());
                                ...
                                if let Lit::Str(lit) = &nv.lit {
                                    ...
                                    let fields_str =
                                        lit.split(',').map(|x| x.trim()).collect::<Vec<_>>();
                                    version_fields = super::check_fields(&fields_str, all_fields.clone());
                ...
                if let Some(version_field) = version_fields.get(0) {
                    if super::contains(&on_fields, version_field) || super::contains(&by_fields, version_field) {
                        panic!("Version field {:?} must not be in 'on'  fields or 'by' fields"
                            , version_field.ident.as_ref().map(|x| x.to_string()).unwrap_or_default()
                        );
                    }
                ...
                if !intersection.is_empty() {
                    let intersection = intersection.iter()
                ...
                if on_fields.is_empty() {
                    ...
                    let (fn_name, fn_name_return) = if let Some(fn_name) = fn_name_attr {
                        ...
                        (format!("update_by_{}", func_name_by_field), format!("update_by_{}_return", func_name_by_field))
                    };
                    ...
                    let set_fields = all_fields
                        .iter()
                        .filter(|x| !super::contains(&by_fields, **x) && !super::contains(&version_fields, **x))
                        .collect::<Vec<_>>();
                    ...
                    let (fn_name, fn_name_return) = if let Some(fn_name) = fn_name_attr {
                        ...
                        (format!("update_by_{}_on_{func_name_on_field}", func_name_by_field), format!("update_by_{}_on_{func_name_on_field}_return", func_name_by_field))
                    };
                    let fn_name = Ident::new(&fn_name, proc_macro2::Span::call_site());
                    let fn_name_return = Ident::new(&fn_name_return, proc_macro2::Span::call_site());
                    let mut fn_args = by_fields
