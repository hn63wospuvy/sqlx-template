use sqlx_template::{MysqlTemplate, mysql_query};
use sqlx::{FromRow, MySqlPool};

#[derive(MysqlTemplate, FromRow, Debug, Clone, Default)]
#[table("users")]
#[tp_select_builder]
...
pub async fn test_null_value_handling() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Testing MySQL NULL Value Handling ===\n");
    
    // Create MySQL database connection
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "mysql://root:password@localhost/test".to_string());
    ...
    User::upsert_by_id(&user1, &pool).await?;
    ...
    User::upsert_by_id(&user2, &pool).await?;
    ...
    User::upsert_by_id(&user3, &pool).await?;
    ...
    User::upsert_by_id(&user4, &pool).await?;
    ...
    User::upsert_by_id(&user5, &pool).await?;
    ...
    println!("\n✅ All MySQL NULL value handling tests passed!");
    println!("📋 Summary of techniques used (following priority rules):");
    println!("   1. ✅ Derive macros (#[tp_upsert], #[tp_select_*]) - HIGHEST PRIORITY");
    println!("   2. ✅ Builder pattern (User::builder_select()) - MEDIUM PRIORITY");
    println!("   3. ✅ Query macros (#[mysql_query]) - LOW PRIORITY");
